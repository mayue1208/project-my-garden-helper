...
function populateParameters(fromRes, toRes) {
  ...
  const parameters = {
    appId: "__UNI__895054C",
    appName: "植物养护助手",
    appVersion: "1.0.0",
    appVersionCode: "100",
    appLanguage: getAppLanguage(hostLanguage),
    uniCompileVersion: "5.09",
    uniCompilerVersion: "5.09",
    uniRuntimeVersion: "5.09",
...
const onError = {
  args(fromArgs) {
    ...
    if (!app.$vm) {
      ...
      injectHook$1(ON_ERROR, fromArgs, app.$vm.$);
    }
  }
};
const offError = {
...
function injectHook$1(type, hook, target = currentInstance, prepend = false) {
  if (target) {
    if (isRootHook(type)) {
      target = target.root;
    }
...
const createHook = (lifecycle) => (hook, target = currentInstance) => (
  ...
  (!isInSSRComponentSetup || lifecycle === "sp") && injectHook$1(lifecycle, (...args) => hook(...args), target)
);
...
function parseApp(instance, parseAppOptions) {
  ...
  if (onErrorHandlers) {
    onErrorHandlers.forEach((fn) => {
      injectHook$1(ON_ERROR, fn, internalInstance);
    });
...
const injectHook = (type, hook, target, prepend) => {
  if (target) {
    const hooks = target[type] || (target[type] = []);
    const w = hook.__weh || (hook.__weh = (...a) => hook(...a));
    hooks.push(w);
    return w;
  }
};
